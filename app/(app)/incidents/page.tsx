...
export default function IncidentsPage() {
  ...
  const componentNames = getDeduplicatedComponentNames()

  const handleFilterChange = (newFilters: Partial<typeof filters>) => {
    setFilters(newFilters)
    // Fetch after filter change - setFilters already resets page to 1
  ...
  const handlePageChange = (newPage: number) => {
    ...
    fetchIncidentsPage(newPage)
  }

  // Enrich incidents with project and component info
  const enrichedIncidents: EnrichedIncident[] = incidentsPage.map((inc) => {
  ...
  const showEmptyState = incidentsPage.length === 0 && !isPaginating
  ...
  return (
    <>
      ...
      {isPaginating && incidentsPage.length === 0 ? (
        <div style={{ display: 'flex', justifyContent: 'center', padding: 'var(--space-64)' }}>
          <Spinner size={24} />
        </div>
      ) : showEmptyState ? (
        <EmptyState
          ...
          description={
            hasActiveFilters
              ...
              : 'All systems are running smoothly. No incidents to report.'
          }
          ...
          action={
            hasActiveFilters ? (
              <button
                ...
                onClick={() => handleFilterChange({ projectId: null, componentId: null, dateFrom: null, dateTo: null })}
              >
                Clear Filters
              </button>
            ) : undefined
